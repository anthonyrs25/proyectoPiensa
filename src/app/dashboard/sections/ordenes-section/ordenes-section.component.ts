...
import { OrdenTrabajo } from '../../dashboard.models';

@Component({
  selector: 'app-ordenes-section',
  standalone: true,
  imports: [IonicModule, CommonModule, FormsModule],
  templateUrl: './ordenes-section.component.html',
  styleUrls: ['./ordenes-section.component.scss'],
})
export class OrdenesSectionComponent {
  q = '';
  ...
  get filtered(): OrdenTrabajo[] {
    const s = this.q.trim().toLowerCase();
    if (!s) return this.store.ordenes;
    return this.store.ordenes.filter(o =>
      `${o.codigoOrden} ${o.fecha} ${o.codigoCliente} ${o.placa} ${o.generadoPorCedula}`.toLowerCase().includes(s)
    );
  }

  newId(): number {
    const max = this.store.ordenes.reduce((m, x) => Math.max(m, x.codigoOrden), 0);
    return max + 1;
  }
  ...
  edit(o: OrdenTrabajo) {
    this.editingId = o.codigoOrden;
    this.codigoOrden = o.codigoOrden;
    this.fecha = o.fecha.slice(0, 10);
    this.codigoCliente = o.codigoCliente;
    this.placa = o.placa;
    this.generadoPorCedula = o.generadoPorCedula;
    this.itemsServicios = JSON.parse(JSON.stringify(o.itemsServicios));
    this.itemsRepuestos = JSON.parse(JSON.stringify(o.itemsRepuestos));
  }
  ...
  save() {
    ...
    if (!this.generadoPorCedula.trim()) { alert('Selecciona el personal que genera la OT.'); return; }
    ...
    const id = this.codigoOrden || this.newId();
    ...
    const idx = this.store.ordenes.findIndex(x => x.codigoOrden === (this.editingId ?? id));
    if (idx >= 0) this.store.ordenes[idx] = o;
    else this.store.ordenes.unshift(o);

    this.reset();
  }

  remove(id: number) {
    this.store.ordenes = this.store.ordenes.filter(o => o.codigoOrden !== id);
  }
  totalOrden(o: any): string {
  let total = 0;

  for (const i of o.itemsServicios) {
    const s = this.store.servicios.find(x => x.codigoServicio === i.codigoServicio);
    if (s) total += s.precio * i.cantidad;
  }

  for (const i of o.itemsRepuestos) {
    const r = this.store.repuestos.find(x => x.codigoRepuesto === i.codigoRepuesto);
